...
def test_before_agent_callback_greeting(session_service, mock_context):
    ...
    with patch('app.shared_libraries.conversation_callbacks._extract_user_input', return_value="hallo, wie geht's?"):
        # Act
        combined_before_agent_callback(invocation_context)

    # Assert
    assert mock_context.state[const.CURRENT_INTERACTION_TYPE] == const.INTERACTION_GREETING
...
def test_before_agent_callback_question(session_service, mock_context):
    ...
    with patch('app.shared_libraries.conversation_callbacks._extract_user_input', return_value="was kostet eine wohnung?"):
        # Act
        combined_before_agent_callback(invocation_context)

    # Assert
    assert mock_context.state[const.CURRENT_INTERACTION_TYPE] == const.INTERACTION_QUESTION
...
def test_analyze_conversation_tool(mock_llm_analysis, mock_context):
    ...
    result = analyze_conversation_context.func(
        user_input="tell me about prices",
        session_context=mock_context.state
    )
...
def test_memorize_conversation_tool(mock_context):
    ...
    assert mock_context.state[const.USER_PREFERENCES]["financials"]["budget"] == "500000" 