...
def search_vertex_ai_search(
    ...
    if use_answer:
        ...
        payload = {
            ...
            },  # German timezone for real estate context
        }
        if session:
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"[ERROR] Vertex AI Search error {response.status_code}: {response.text}"
                )
                response.raise_for_status()
        ...
        for doc in data.get("results", []):
            ...
            if content or title or link:
                results.append(
                    {"title": title, "content": content, "page": "", "link": link}
                )
