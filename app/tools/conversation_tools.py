...
def analyze_conversation_context(
    ...
    try:
        ...
        analysis = _analyze_user_input_with_llm(
            user_input, conversation_history or [], session_context or {}
        )

        logger.debug(
            f"Conversation analysis completed: {analysis.get('interaction_type', 'unknown')}"
        )
...
def _analyze_user_input_with_llm(
    user_input: str, conversation_history: list, session_context: dict
) -> Dict[str, Any]:
    """
...
def _intelligent_heuristic_analysis(
    user_input: str, session_context: dict
) -> Dict[str, Any]:
    """
    ...
    conversation_phase = _determine_conversation_phase(
        interaction_type, session_context
    )
...
def _determine_interaction_type_intelligent(
    user_input: str, session_context: dict
) -> str:
    """
...
def _extract_topics_intelligent(user_input: str) -> list:
    ...
    if any(
        ...
        for term in ["apartment", "flat", "condo", "квартира", "wohnung"]
    ):
        topics.append("apartment_search")

    if any(
        term in user_input_lower
        for term in ["house", "home", "villa", "дом", "haus", "villa"]
    ):
...
def _extract_preferences_intelligent(user_input: str) -> dict:
    ...
    if any(
        term in user_input_lower for term in ["room", "bedroom", "zimmer", "комнат"]
    ):
        size_match = re.search(
            r"(\d+)\s*(?:room|bedroom|zimmer|комнат)", user_input_lower
        )
        if size_match:
...
def _get_fallback_analysis(user_input: str = "") -> Dict[str, Any]:
    ...
    if any(word in user_input.lower() for word in ["hello", "hi", "привет", "hallo"]):
        ...
        for word in ["thanks", "bye", "спасибо", "пока", "danke"]
    ):
    ...
    return {
        ...
        "memory_updates": {"suggested_topics": [], "user_preferences": {}},
    }


@FunctionTool
def set_conversation_stage(
    stage: str, user_context: Optional[dict] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "error", "message": f"Error setting stage: {str(e)}"}
