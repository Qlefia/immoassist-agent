...
from ..models.user import ConversationHistory, UserInteraction, UserSession
...
class SessionService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"SessionService initialized with {config.session_timeout_minutes}min timeout"
        )
    ...
    def create_session(
        ...
        self._histories[session_id] = ConversationHistory(session_id=session_id)
    ...
    def get_session(self, session_id: str) -> Optional[UserSession]:
        ...
        updated_session = session.copy(update={"last_activity": datetime.now()})
        self._sessions[session_id] = updated_session

        return updated_session

    def update_session_context(self, session_id: str, context_updates: dict) -> bool:
        """
        ...
        updated_session = session.copy(
            update={"context": new_context, "last_activity": datetime.now()}
        )
    ...
    def get_conversation_history(
        ...
        if last_n:
            ...
            history = history.copy(update={"interactions": recent_interactions})
    ...
    def get_session_stats(self) -> dict:
        ...
        active_sessions = sum(
            1 for s in self._sessions.values() if not self._is_session_expired(s)
        )
